...
func NewHTTPStaticServer(root string, noIndex bool) *HTTPStaticServer {
	...
	//m.NewRoute().Subrouter().HandleFunc("/up", s.up).Methods("GET")
	// routers for Apple *.ipa
	router.HandleFunc("/-/ipa/plist/{path:.*}", s.hPlist)
	router.HandleFunc("/-/ipa/link/{path:.*}", s.hIpaLink)
...
func (s *HTTPStaticServer) up(w http.ResponseWriter, r *http.Request) {
	// 设置响应头
